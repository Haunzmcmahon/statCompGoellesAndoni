...
{{< pagebreak >}} \## Descriptive Analysis of the Sample
...
-   **Region** (`region_name`): Residents of Vienna and Lower Austria receive significantly higher pensions compared to those in Burgenland:
...
{{< pagebreak >}} \### Diagnostic Checks for Final Model
...
```{r}
scatterplot(old_age_benefits ~ age_years, data = cleaned_data_no_zeros, smooth = list(style = "lines"), col = 1)
...
#| fig.height: 5
#| 
pairs(cleaned_data_no_zeros[, c("old_age_benefits", "age_years", "household_size")], panel = panel.smooth)
...
## Interaction Terms and Higher-Order Models

```{r}
lm_high_interaction <- lm(old_age_benefits ~ age_years * region_name * economic_status, data = cleaned_data_no_zeros)
# summary(lm_high_interaction)
```

{{< pagebreak >}} \## Refining the Model with a New Predictor

### Selecting a New Predictor
...
### Testing `household_size` as a Predictor
...
{{< pagebreak >}} 
## Refined Model: Main Income Earner and Household Size
...
To capture potential joint effects, include an interaction term between household_size and main_income_earner.
...
### Testing Additional Predictor region_name
...
- The model explains 16.68% of the variance in old_age_benefits (Multiple $R^2$ = 0.1668.
- The adjusted $R^2$ is slightly lower at 16.48%, indicating modest explanatory power.
- The overall model is statistically significant (p<2.2×10−16).
...
$B = \beta_{0} + \beta_{1}*(Household Size) + \beta_{2}*Main Income Earner+\beta_{3}*Region Name+\beta_{4}(Household Size×Main Income Earner)+\epsilon$

Where:

- B:  BoxCox Transformed Old Age Benefits
- $\beta_{0}$ : Intercept
- $\beta_{1}$, $\beta_{2}$, $\beta_{3}$, $\beta_{4}$  : Coefficients for predictors and interactions
...
```{r}
#| tbl-cap: "Key Results from the Final Model"
#| echo: false
#| code-fold: false

# Create a tibble for the key results
key_results_final <- tibble::tibble(
  Predictor = c(
    "Intercept",
    "Household Size",
    "Main Income Earner (TRUE)",
    "Region: Lower Austria",
    "Region: Vienna",
    "Household Size × Main Income Earner (TRUE)"
  ),
  Coefficient = c(
    105.33,
    1.80,
...
knitr::kable(key_results_final, caption = "Key Results from the Final Model") |>
  kableExtra::kable_styling(full_width = TRUE, position = "center") |>
  kableExtra::column_spec(1, width = "12em" ,bold = TRUE) |>
...
```{r}
#| tbl-cap: "Confidence Intervals for Model Coefficients"
#| echo: false
#| code-fold: false

# Generate confidence intervals for the model
conf_intervals <- confint(lm_boxcox)

# Create a tibble for confidence intervals
confidence_table <- tibble::tibble(
  Predictor = rownames(conf_intervals),
  `Lower Bound (2.5%)` = round(conf_intervals[, 1], 2),
  `Upper Bound (97.5%)` = round(conf_intervals[, 2], 2)
)

# Display the table with formatting
knitr::kable(confidence_table, caption = "Confidence Intervals for Model Coefficients") |>
  kableExtra::kable_styling(full_width = FALSE, position = "center") |>
...
- **Observations**:
  - The interaction between household_size and main_income_earner shows a steeper increase in benefits for main income earners as household size grows.
