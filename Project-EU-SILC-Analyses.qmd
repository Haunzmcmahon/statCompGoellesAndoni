...
```{r}
kde <- kde2d(cleaned_data_no_zeros$age_years, cleaned_data_no_zeros$old_age_benefits, n = 250)
filled.contour(kde)
```
...
>>>>>>> 70bce63f2a7a94b364089cb84f3e8f9a469f3f5f

## Effect Plots for Interactions

```{r}
library(effects)
effect_plot <- allEffects(lm_interaction)
plot(effect_plot, multiline = TRUE)
```
